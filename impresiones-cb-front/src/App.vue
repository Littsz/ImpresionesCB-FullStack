    ...
    <div class="card shadow-lg border-0">
      ...
      <div class="card-body bg-light text-center p-5">
        ...
        <div v-if="inventario.length > 0" class="animate__animated animate__fadeIn">
          ...
          <div class="table-responsive shadow-sm rounded">
            <table class="table table-hover table-striped mb-0">
              <thead class="table-primary">
                <tr><th class="py-3">#</th><th class="py-3">Código detectado</th></tr>
              </thead>
              <tbody>
          ...
          <button class="btn btn-success btn-lg mt-4 w-100 py-3 fw-bold shadow">
            🖨️ GENERAR Y ENVIAR A IMPRIMIR
          </button>
        </div>
...
const leerExcel = (event) => {
  ...
  reader.onload = (e) => {
    ...
    inventario.value = rows.flat().filter(cell => cell != null && cell.toString().trim() !== "");
  };
  reader.readAsArrayBuffer(file);
};

const guardarConfig = () => {
  console.log("Configuración actualizada:", config.value);s
};
</script>
